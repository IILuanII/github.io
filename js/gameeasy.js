var Game = Game || (function () {
    ...
    function manualAttack() {
        ...
        if (!isNaN(x) && x >= 1 && x <= 5 && _letters.includes(y) && y != "") {
            var row = y;
            var col = x;
            var $cell = $rows.filter('[data-row="' + row + '"]').find('td[data-col="' + col + '"]');
            attack($cell, row, col);
        } else {
            alert("Coordenadas inválidas! Por favor, insira valores válidos para Y (A-J) e X (1-5).");
        }
    }
    ...
    function can_set_ship(length) {
        ...
        if (_orientation == 1) { // vertical
            ...
            _direction = (_row + length - 1 <= 5) ? 1 : -1;
            ...
            _direction = (_col + length - 1 <= 5) ? 1 : -1;
    ...
    function showNextPosition() {
        if (_currentShipIndex < allShips.length) {
            ...
            // Obter todas as coordenadas do navio
            var coordinates = positions.map(function (position) {
                return position.row + position.col; // Concatena a linha e coluna, ex: A1, B3, etc.
            });
    
            // Exibir todas as posições do navio
            var positionText = `O navio está nas posições: ${coordinates.join(", ")}.`;
